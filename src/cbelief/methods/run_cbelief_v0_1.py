...
def bool_flag(quality: Dict[str, Any], key: str) -> bool:
    return bool(quality.get(key, False))
...
def score_visible_evidence(sample: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bool_flag(quality, "stable_high_creatinine_level"):
        chronic_score += 1.25
...
def determine_claim_status(scores: Dict[str, Any], primary: str) -> str:
    ...
    # Strong uncertainty dominates
    if uncertain >= max(acute, chronic) and acute < 1.0 and chronic < 1.0:
        return "insufficient"

    # Mixed acute/chronic evidence should be partial
    if acute > 0 and chronic > 0:
        return "partially_supported"

    # Low-confidence acute pattern should be partial
    if acute > 0 and any("low_confidence" in r for r in partial_reasons):
        return "partially_supported"

    # Comorbid or future-only subtypes should be partial/insufficient, not overcalled
    if any("comorbid" in r for r in partial_reasons):
        return "partially_supported"
...
def verify_claim(sample: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prediction = {
        ...
        "method_name": "cbelief_v0_1_rule_skill",
