...
def determine_claim_status(scores: Dict[str, Any], primary: str) -> str:
    ...
    has_future_only_pattern = any("future_" in r for r in partial_reasons)
    has_low_confidence_pattern = any("low_confidence" in r for r in partial_reasons)
    has_stable_chronic_pattern = "stable_high_creatinine_level" in partial_reasons
    has_comorbid_pattern = any("comorbid" in r for r in partial_reasons)

    acute_support_ids = scores.get("acute_supporting_ids", [])
    chronic_support_ids = scores.get("chronic_supporting_ids", [])

    has_explicit_chronic_support = len(chronic_support_ids) > 0
    ...
    if has_future_only_pattern:
        return "insufficient"

    # Low-confidence acute rise should be partial
    if has_low_confidence_pattern:
        return "partially_supported"

    # Stable high creatinine suggests chronic dysfunction,
    # but without explicit pre-query CKD text/code, claim support should be partial.
    if primary == "chronic_renal_dysfunction":
        if has_stable_chronic_pattern and not has_explicit_chronic_support:
            return "partially_supported"
        if chronic >= 1.0:
            return "supported"

    # Mixed acute/chronic evidence should be partial only when the scores are close.
    if acute > 0 and chronic > 0 and margin < 0.75:
        return "partially_supported"

    # Comorbidity alone should not force partial if one hypothesis dominates.
    if has_comorbid_pattern and margin < 0.75:
        return "partially_supported"

    # Clear acute dominance
    if primary == "acute_renal_deterioration" and acute >= 1.0:
        return "supported"

    # Clear chronic dominance
    if primary == "chronic_renal_dysfunction" and chronic >= 1.0:
        return "supported"
...
def verify_claim(sample: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prediction = {
        ...
        "method_name": "cbelief_v0_2_2_chronic_claimfix",
...
if __name__ == "__main__":
    run(
        ...
        "data/predictions/cbelief/test_predictions_v0_2_2_chronic_claimfix.jsonl",
    )