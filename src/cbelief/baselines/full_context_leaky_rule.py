...
def predict(sample: Dict[str, Any]) -> Dict[str, Any]:
    # Leaky baseline: intentionally uses all evidence
    all_events = (
        sample.get("visible_events", [])
        + sample.get("future_observed_events", [])
        + sample.get("retrospective_events", [])
    )
    ...
    return {
        ...
        "method_name": "full_context_leaky_rule",
        ...
        "missing_evidence": [],
        "rationale": (
            "Full-context leaky rule baseline used visible, future, and retrospective "
            "evidence together without temporal gatekeeping."
        ),
    }
