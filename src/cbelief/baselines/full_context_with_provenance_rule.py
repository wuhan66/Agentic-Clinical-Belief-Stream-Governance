...
def predict(sample: Dict[str, Any]) -> Dict[str, Any]:
    # Full context is available, but provenance labels are respected.
    visible_events = sample.get("visible_events", [])
    future_events = sample.get("future_observed_events", [])
    retrospective_events = sample.get("retrospective_events", [])

    acute_ids = []
    chronic_ids = []

    # Only visible events are allowed for query-time support.
    ...
    return {
        ...
        "method_name": "full_context_with_provenance_rule",
        "claim_status": claim_status,
        "primary_hypothesis": primary,
        "belief_distribution": belief,
        "supporting_evidence_ids": supporting_ids,
        "contradictory_evidence_ids": [],
        "retrospective_only_evidence_ids": [e["event_id"] for e in retrospective_events],
        "future_only_evidence_ids": [e["event_id"] for e in future_events],
        "misused_future_evidence_ids": [],
        "missing_evidence": [],
        "rationale": (
            "Full context was available, but provenance labels were used to prevent "
            "future and retrospective evidence from supporting the query-time claim."
        ),
    }
...
if __name__ == "__main__":
    run(
        ...
        "data/predictions/full_context_with_provenance_rule/test_predictions.jsonl",
    )